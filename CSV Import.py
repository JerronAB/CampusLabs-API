...
AdvisorList = API.CSVTableSubclass(envDict['advisor_list'])
...
for filename in importableCSVs: #can't seem to use this w/ lambda and map
    csvObj = API.CSVTableSubclass(filename)
    SQLTable = API.migrateData(csvObj,"sql")
    SQLTable.dataPush()

print('Importing and setting advisor count...')
adv_counts = API.SQLTableSubclass(db_tablename='advisor_assignment_count')
...
exemptions = API.CSVTableSubclass(envDict['exemptions'])
...
all_assignments = API.SQLTableSubclass(db_tablename='all_assignments')
...
unfiltered_export = API.migrateData(all_assignments,"csv")
...
filtered_export = API.CSVTableSubclass()
